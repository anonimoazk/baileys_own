import makeWASocket, {
  DisconnectReason,
  useMultiFileAuthState,  // ← MANTER como fallback
  WAConnectionState,
  WASocket,
...
export class BaileysInstance implements IBaileysInstance {
  private logger: pino.Logger;
  instanceId: string;
  qr: string | null = null;
  status: { connection: WAConnectionState } = { connection: 'close' };
  ...
  private deleteSessionFn: (() => Promise<void>) | null = null;

  // ── Rate-limit backoff (Code 405 from WhatsApp = connection blocked) ──
  private consecutive405Count = 0;
  private readonly MAX_405_RETRIES = 5;
  private readonly BASE_405_DELAY_MS = 30000; // 30s base → 30, 60, 120, 240, 480s
    ...
    if (process.env.REDIS_URL || process.env.REDIS_HOST) {
      try {
        ...
        const result = await useMultiFileAuthState(
          path.join(SESSION_PATH, this.instanceId)
        );
        state = result.state;
        saveCreds = result.saveCreds;
      ...
      try {
        for (const msg of m.messages) {
          ...
          for (const type of mediaTypes) {
            if (messageContent[type as keyof typeof messageContent]) {
              hasMedia = true;
              mediaType = type;
              break;
            }
          ...
          if (hasMedia) {
            ...
            try {
              ...
              const mediaMsg = messageContent[mediaType as keyof typeof messageContent] as any;
  ...
  async destroy(logout: boolean = false): Promise<void> {
